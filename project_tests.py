...
df = pd.read_csv(r"/Users/rli/Documents/Downloads/new study/udacity/ibm/user-item-interactions.csv")
df_content = pd.read_csv(r"/Users/rli/Documents/Downloads/new study/udacity/ibm/articles_community.csv")
del df['Unnamed: 0']
...
def sol_4_test(sol_4_dict):
    ...
    if sol_4_dict == sol_4_dict_1:
        print("Awesome job!  That's right!  All of the test article are in the training data, but there are only 20 test users that were also in the training set.  All of the other users that are in the test set we have no data on.  Therefore, we cannot make predictions for these users using SVD.")
    else:
